...
*   **Document/Text Content (CRITICAL PAGE RULE):**
    *   **Multi-Page Separation:** If the input is a multi-page document (e.g., PDF, presentation), you **MUST** use the "[Page: X]" tag as the first line of content for each page.
    *   **Single Image Input:** If the input is a single, standalone image, **DO NOT** use the "[Page: X]" tag. Start directly with the content extraction.
    *   **Header/Footer Extraction:** The first element extracted *after* the "[Page: X]" tag (or the very first element for a single image) **MUST** be the document header, and the final elements **MUST** be the document footer/end-notes. The company logo/icon in the header **MUST** be extracted using the "[Logo: <description>]" tag, preceded by a location tag.
    ...
    *   **Embedded Components:** If the document contains an image or chart, immediately switch to and apply the **Image Content Rules** for that component.
...
*   **TERMINATION RULE:** After completing the extraction of the final element, you **MUST** stop generating output immediately.`
