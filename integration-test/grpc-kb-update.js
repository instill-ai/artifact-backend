...
export function teardown(data) {
    ...
    group(groupName, () => {
        ...
        console.log("Step 2: Force-clearing workflow IDs and waiting for cleanup...");
        try {
        ...
        console.log("Step 3: Direct DB cleanup of all test data...");
        try {
            // Clean up in reverse dependency order
            const chunkRows = constant.db.exec(`DELETE FROM text_chunk WHERE file_uid IN (SELECT uid FROM knowledge_base_file WHERE name LIKE '${constant.dbIDPrefix}%')`);
            console.log(`Cleaned ${chunkRows} text_chunk rows`);

            const embeddingRows = constant.db.exec(`DELETE FROM embedding WHERE file_uid IN (SELECT uid FROM knowledge_base_file WHERE name LIKE '${constant.dbIDPrefix}%')`);
            console.log(`Cleaned ${embeddingRows} embedding rows`);

            const convertedRows = constant.db.exec(`DELETE FROM converted_file WHERE file_uid IN (SELECT uid FROM knowledge_base_file WHERE name LIKE '${constant.dbIDPrefix}%')`);
            console.log(`Cleaned ${convertedRows} converted_file rows`);

            const fileRows = constant.db.exec(`DELETE FROM knowledge_base_file WHERE name LIKE '${constant.dbIDPrefix}%'`);
            console.log(`Cleaned ${fileRows} knowledge_base_file rows`);

            const kbRows = constant.db.exec(`DELETE FROM knowledge_base WHERE id LIKE '${constant.dbIDPrefix}%'`);
            console.log(`Cleaned ${kbRows} knowledge_base rows`);
        } catch (e) {
...
function TestCC01_AddingFilesDuringSwap(client, data) {
    ...
    group(groupName, () => {
        ...
        const updateCompletedCC1 = helper.pollUpdateCompletion(client, data, catalogUidCC1, 600);
...
function TestCC02_DeletingFilesDuringSwap(client, data) {
    ...
    group(groupName, () => {
        ...
        const updateCompleted = helper.pollUpdateCompletion(client, data, catalogUidCC2, 600);
...
function TestCC03_RapidOperations(client, data) {
    ...
    group(groupName, () => {
        ...
        const updateCompleted = helper.pollUpdateCompletion(client, data, catalogUidCC3, 600);
...
function TestCC04_RaceConditions(client, data) {
    ...
    group(groupName, () => {
        ...
        const updateCompletedCC4 = helper.pollUpdateCompletion(client, data, catalogUidCC4, 600);
...
function TestCC05_AddingFilesAfterSwap(client, data) {
    ...
    group(groupName, () => {
        ...
        const updateCompletedCC5 = helper.pollUpdateCompletion(client, data, catalogUidCC5, 600);
...
function TestCC06_DeletingFilesAfterSwap(client, data) {
    ...
    group(groupName, () => {
        ...
        const updateCompletedCC6 = helper.pollUpdateCompletion(client, data, catalogUidCC6, 600);
...
function TestCC07_MultipleOperations(client, data) {
    ...
    group(groupName, () => {
        ...
        const updateCompletedCC7 = helper.pollUpdateCompletion(client, data, catalogUidCC7, 600);
...
function TestCC08_RollbackDuringProcessing(client, data) {
    ...
    group(groupName, () => {
        ...
        const updateCompletedCC8 = helper.pollUpdateCompletion(client, data, catalogUidCC8, 600);
...
function TestCC09_DualProcessingStops(client, data) {
    ...
    group(groupName, () => {
        ...
        // Wait for completion
        const updateCompletedCC9 = helper.pollUpdateCompletion(client, data, catalogUidCC9, 600);
...
function TestCC10_RetentionExpiration(client, data) {
    ...
    group(groupName, () => {
        ...
        // Wait for completion
        const updateCompletedCC10 = helper.pollUpdateCompletion(client, data, catalogUidCC10, 600);
        ...
        // Wait for retention to expire (20s total wait = 5s retention + 15s buffer for cleanup activities)
        // The cleanup workflow needs time to: sleep 5s + execute 8 cleanup activities + soft-delete KB
        // Increased buffer to handle resource contention during parallel test execution
        console.log("CC10: Waiting for retention to expire and cleanup to complete (20s)...");
        sleep(20);
...
function TestResourceCleanup(client, data) {
    ...
    group(groupName, () => {
        ...
        const secondUpdateCompleted = helper.pollUpdateCompletion(client, data, catalogUid, 600);
...
function TestRollbackAndReUpdate(client, data) {
    ...
    group(groupName, () => {
        ...
        const firstUpdateCompleted = helper.pollUpdateCompletion(client, data, catalogUid, 600);
        ...
        // Increased timeout for CI environments (updates after rollback should be faster but still need margin)
        const secondUpdateCompleted = helper.pollUpdateCompletion(client, data, currentCatalogUid, 180);
        ...
        // Increased timeout for CI environments (updates after rollback should be faster but still need margin)
        const thirdUpdateCompleted = helper.pollUpdateCompletion(client, data, originalKBUID, 180);
...
function TestMultipleKBUpdates(client, data) {
    ...
    group(groupName, () => {
        ...
        let maxWaitSeconds = 240; // Extended for CI (was 120s) - processing 3 files across 3 KBs in parallel
        let waitedSeconds = 0;
        ...
        while (iterations < maxIterations && completedKBs < numKBs) {
            ...
            for (let i = 0; i < numKBs; i++) {
                ...
                if (kb && kb.length > 0 && kb[0].update_status === "KNOWLEDGE_BASE_UPDATE_STATUS_COMPLETED") {
                    completedKBs++;
                }
            }
