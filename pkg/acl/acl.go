...
// Note: the upstream `x.ACLClient.ListPermissions` API dropped the
// `isPublic` boolean; globally-public listings now go through
// `ListPublicPermissions` in the `x` package. Callers that need the
// wildcard view must switch explicitly (see PR instill-ai/x#89) — the
// split avoids silent fallbacks to the public set when caller identity
// fails to resolve.
