...
func NewTempCatalogFileToEmbWorkerPool(ctx context.Context, svc *service.Service, nums int, catalogType artifactpb.CatalogType) *tempCatalogFileToEmbWorkerPool {
	ctx, cancel := context.WithCancel(ctx)
	return &tempCatalogFileToEmbWorkerPool{
...
func (wp *tempCatalogFileToEmbWorkerPool) Start() {
...
func (wp *tempCatalogFileToEmbWorkerPool) startDispatcher() {
	logger, _ := logger.GetZapLogger(wp.ctx)
	defer wp.wg.Done()
...
func (wp *tempCatalogFileToEmbWorkerPool) startWorker(ctx context.Context, workerID int) {
...
// stop
func (wp *tempCatalogFileToEmbWorkerPool) GraceFulStop() {
	logger, _ := logger.GetZapLogger(wp.ctx)
...
func (wp *tempCatalogFileToEmbWorkerPool) processFile(ctx context.Context, file repository.KnowledgeBaseFile) error {
	logger, _ := logger.GetZapLogger(ctx)
	...
	for {
		...
		case artifactpb.FileProcessStatus_FILE_PROCESS_STATUS_CHUNKING:
			t0 := time.Now()
			updatedFile, nextStatus, err := wp.processChunkingFilesInTempCatalog(ctx, file)
			if err != nil {
...
func (wp *tempCatalogFileToEmbWorkerPool) processWaitingFile(ctx context.Context, file repository.KnowledgeBaseFile) (updatedFile *repository.KnowledgeBaseFile, nextStatus artifactpb.FileProcessStatus, err error) {
...
func (wp *tempCatalogFileToEmbWorkerPool) processConvertingFile(ctx context.Context, file repository.KnowledgeBaseFile) (updatedFile *repository.KnowledgeBaseFile, nextStatus artifactpb.FileProcessStatus, err error) {
	logger, _ := logger.GetZapLogger(ctx)
...
// Finally, the file status is updated to chunking in the database.
// If the file is not a PDF, it returns an error.
func (wp *tempCatalogFileToEmbWorkerPool) procesSummarizingFile(ctx context.Context, file repository.KnowledgeBaseFile) (updatedFile *repository.KnowledgeBaseFile, nextStatus artifactpb.FileProcessStatus, err error) {
	logger, _ := logger.GetZapLogger(ctx)
...
// processChunkingFilesInTempCatalog processes a file in "chunking" status by splitting it into text chunks.
// The processing varies by file type:
//
// For document files (PDF, DOC, DOCX, PPT, PPTX, HTML, XLSX, XLS, CSV):
// - Retrieves the converted markdown file from MinIO
// - Uses text chunking pipeline to split content
//
// For TEXT files:
...
//
// For all file types:
// - Saves chunks to MinIO object storage
...
func (wp *tempCatalogFileToEmbWorkerPool) processChunkingFilesInTempCatalog(ctx context.Context, file repository.KnowledgeBaseFile) (*repository.KnowledgeBaseFile, artifactpb.FileProcessStatus, error) {
	logger, _ := logger.GetZapLogger(ctx)
	...
	case artifactpb.FileType_FILE_TYPE_PDF.String(),
		...
		contentChunks, err = wp.svc.ChunkMarkdownPipeForTempCatalog(ctx, file.CreatorUID, requesterUID, string(fileData))
		if err != nil {
	...
	case artifactpb.FileType_FILE_TYPE_TEXT.String():
		...
		contentChunks, err = wp.svc.ChunkTextPipeForTempCatalog(ctx, file.CreatorUID, requesterUID, string(fileData))
		if err != nil {
			logger.Error("Failed to get chunks from file.", zap.String("File uid", file.UID.String()))
			return nil, artifactpb.FileProcessStatus_FILE_PROCESS_STATUS_UNSPECIFIED, err
		}
	...
	summaryChunks, err := wp.svc.ChunkTextPipeForTempCatalog(ctx, file.CreatorUID, requesterUID, string(file.Summary))
	if err != nil {
		logger.Error("Failed to get chunks from file.", zap.String("File uid", file.UID.String()))
		return nil, artifactpb.FileProcessStatus_FILE_PROCESS_STATUS_UNSPECIFIED, err
	}
...
func (wp *tempCatalogFileToEmbWorkerPool) processEmbeddingFile(ctx context.Context, file repository.KnowledgeBaseFile) (updatedFile *repository.KnowledgeBaseFile, nextStatus artifactpb.FileProcessStatus, err error) {
	logger, _ := logger.GetZapLogger(ctx)
...
func (wp *tempCatalogFileToEmbWorkerPool) processCompletedFile(ctx context.Context, file repository.KnowledgeBaseFile) error {
	logger, _ := logger.GetZapLogger(ctx)
