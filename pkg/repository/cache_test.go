...
func TestCacheKey(t *testing.T) {
	...
	tests := []struct {
	...
	}{
		{
			...
			wantKey:  "artifact:cache:kb:12345678-1234-1234-1234-123456789012:files:2f44fb8c7d56ab14", // Hash of file1
		},
		{
			name:     "multiple files sorted",
			kbUID:    kbUID,
			fileUIDs: []types.FileUIDType{file1, file2, file3},
			wantKey:  "artifact:cache:kb:12345678-1234-1234-1234-123456789012:files:7c3e9c5c5e8a9e7d", // Hash of sorted files
		},
		{
			name:     "deterministic regardless of input order",
			kbUID:    kbUID,
			fileUIDs: []types.FileUIDType{file3, file1, file2},                                        // Different order
			wantKey:  "artifact:cache:kb:12345678-1234-1234-1234-123456789012:files:7c3e9c5c5e8a9e7d", // Same hash
		},
	}

	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			got := CacheKey(tt.kbUID, tt.fileUIDs)

			// Check prefix
			assert.Contains(t, got, "artifact:cache:kb:")
...
func TestCacheKey_Determinism(t *testing.T) {
	...
	key1 := CacheKey(kbUID, []types.FileUIDType{file1, file2, file3})
	key2 := CacheKey(kbUID, []types.FileUIDType{file3, file1, file2})
	key3 := CacheKey(kbUID, []types.FileUIDType{file2, file3, file1})
...
func TestCacheKey_DifferentFiles(t *testing.T) {
	...
	key1 := CacheKey(kbUID, []types.FileUIDType{file1, file2})
	key2 := CacheKey(kbUID, []types.FileUIDType{file1, file3})
	key3 := CacheKey(kbUID, []types.FileUIDType{file2, file3})
...
func TestCacheKey_DifferentKBs(t *testing.T) {
	...
	key1 := CacheKey(kb1, []types.FileUIDType{file1, file2})
	key2 := CacheKey(kb2, []types.FileUIDType{file1, file2})
