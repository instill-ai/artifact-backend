...
// TranspileFilter transpiles an AIP-160 filter expression into a GORM clause
func (r *repository) TranspileFilter(filter filtering.Filter) (*clause.Expr, error) {
	t := &transpiler{filter: filter}
	return t.Transpile()
}
...
func (t *transpiler) transpileComparisonCallExpr(e *expr.Expr, op interface{}) (*clause.Expr, error) {
	...
	case clause.Eq:
		switch ident.SQL {
		case "q":
			sql = "((word_similarity(LOWER(?), LOWER(display_name)) >= 0.3) OR (LOWER(display_name) LIKE LOWER(?)) OR (SIMILARITY(LOWER(id), LOWER(?)) > 0.2) OR (LOWER(id) LIKE LOWER(?)) OR (LOWER(description) LIKE LOWER(?)))"
			pattern := fmt.Sprintf("%%%s%%", con.Vars[0])
			vars = append(vars, con.Vars[0], pattern, con.Vars[0], pattern, pattern)
