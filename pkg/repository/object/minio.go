...
func (m *minioStorage) SaveConvertedFile(ctx context.Context, kbUID types.KBUIDType, fileUID types.FileUIDType, convertedFileUID types.ConvertedFileUIDType, fileExt string, content []byte) (string, error) {
	...
	case "mp4":
		mimeType = "video/mp4"
	case "webp":
		// CONVERTED_FILE_TYPE_THUMBNAIL: 1024 px-wide preview generated by
		// GenerateThumbnailActivity. Served through
		// `/v1alpha/blob-urls/{object_uid}` and therefore Cloudflare-cacheable.
		mimeType = "image/webp"
	default:
