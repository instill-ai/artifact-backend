...
func ChunkMarkdownPipe(ctx context.Context, pipelineClient pipelinepb.PipelinePublicServiceClient, markdown string) (*TextChunkResult, error) {
	...
	var filteredResult []types.TextChunk
...
// GenerateSummaryPipe triggers the generate summary pipeline
func GenerateSummaryPipe(ctx context.Context, pipelineClient pipelinepb.PipelinePublicServiceClient, content string, fileType artifactpb.FileType) (string, error) {
	ctx, cancel := context.WithTimeout(ctx, 300*time.Second)
	defer cancel()
	req := &pipelinepb.TriggerNamespacePipelineReleaseRequest{
